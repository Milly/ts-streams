...
import { mergeAll } from "./merge_all.ts";
...
describe("mergeAll()", () => {
  ...
  describe("returns a TransformStream and", () => {
    it("emits each chunk of all higher-order ReadableStream in merged", async () => {
      await testStream(async ({ readable, assertReadable }) => {
        ...
        const expected = "        -ABC--E--GF-H--I--|";

        const actual = source.pipeThrough(mergeAll());
    ...
    it("terminates when the readable side cancels", async () => {
      await testStream(async ({ readable, writable, run, assertReadable }) => {
        ...
        await assertReadable(source, expectedSource, { a }, "abort");
        await assertReadable(a, expectedA, {}, "abort");
      });
    });
  });
});
